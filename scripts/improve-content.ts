...
const formatContentMarkdown = async (
  content: string,
): Promise<string> => {
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a helpful assistant that formats text content using Markdown. Use elements like headlines, bold text, lists, etc., where appropriate to improve readability. **Do not rewrite or change the actual content, only apply formatting.** Respond only with the formatted Markdown content.',
      },
      {
        role: 'user',
...
${content}`,
  ...
  return result; // Return the raw Markdown string
};
