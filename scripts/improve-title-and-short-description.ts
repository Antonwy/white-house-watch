...
for (const article of articlesWithContent) {
  ...
  await db.update(articlesSchema).set({ title, shortDescription }).where(eq(articlesSchema.id, article.id));
}
...
// const { title, shortDescription } = await improveTitleAndCreateShortDescription('Presidential Memorandum on Combating Government Censorship and Misuse of Authority', content);
