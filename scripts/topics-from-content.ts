...
console.log(`Processing articles in batches of ${BATCH_SIZE}...`);

for (let i = 0; i < articles.length; i += BATCH_SIZE) {
  const batch = articles.slice(i, i + BATCH_SIZE);
  const batchNumber = Math.floor(i / BATCH_SIZE) + 1;
  console.log(`Processing batch ${batchNumber} with ${batch.length} articles...`);

  const batchUpdatePromises = [];

  for (const article of batch) {
    if (!article.resourceId) {
      console.warn(`  Article ${article.id} has no resourceId, skipping.`);
      continue;
    }
    const content = await db.query.resourcesSchema.findFirst({
      where: eq(resourcesSchema.id, article.resourceId),
    });
    const generatedTopics = await topicsFromContent(content?.content || '');

    batchUpdatePromises.push(
      db.update(articlesSchema).set({ topics: generatedTopics }).where(eq(articlesSchema.id, article.id))
    );
  }
